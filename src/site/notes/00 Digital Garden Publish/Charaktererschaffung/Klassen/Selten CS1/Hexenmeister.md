---
{"dg-publish":true,"dg-path":"Charaktererschaffung/Klassen/Selten CS1/Hexenmeister.md","permalink":"/Charaktererschaffung/Klassen/Selten CS1/Hexenmeister/","tags":["klassen/selten","klassen/cs1"],"dg-note-properties":{"tags":["klassen/selten","klassen/cs1"]}}
---


## Hexenmeister 

*Heulende Krieger mit geschärften Zähnen, wildäugige Verkünder des Jüngsten Gerichts, welche die Auflösung (The Dissolution) predigen, und in Mäntel gehüllte Wissensjäger, die das verborgene Mal von Shune tragen.*


**Waffen:** Knüppel, Armbrust, Dolch, Streitkolben, Langschwert

**Rüstung:** Lederrüstung, Kettenhemd und Schilde

**Trefferpunkte:** 1d6 pro Level

**Sprachen:** Wähle eine der folgenden: Celestisch, Diabolisch, Drakonisch, Primordial oder Sylvanisch


### Fähigkeiten

#### Patron

Wähle einen Patron, dem du dienst (siehe S. 17). Dein Patron ist die Quelle deiner übernatürlichen Gaben.

Dein Patron kann entscheiden, dir seine Gaben jederzeit zu gewähren oder vorzuenthalten. Als Folge davon kannst du neue Patronssegen/Talente erlangen (oder verlieren).

#### Patronssegen

Auf Level 1 erhältst du einen zufälligen Patronssegen von der Tabelle deines gewählten Patrons (siehe [[00 Digital Garden Publish/Charaktererschaffung/Klassen/Selten CS1/Hexenmeister#Patrone\|#Patrone]]).

Wann immer du beim Stufenaufstieg ein neues Talent auswürfelst, kannst du dich entscheiden, auf deiner Patronssegen-Tabelle anstelle der regulären Talenttabelle des Hexenmeisters zu würfeln.


### Talente
*(Würfel ein neues Talent, wenn du Level 1, 3, 5, 7, 9 erreichst)*

| 2d6 | Effekt |
| :--- | :--- |
| 2 | Würfle einen Patronssegen von einem *beliebigen* Patron aus – ein unerklärliches Geschenk. |
| 3-6 | Erhöhe zwei verschiedene Attribute deiner Wahl um jeweils +1 Punkt. |
| 7-9 | +1 auf Nahkampf- oder Fernkampfangriffe. |
| 10-11 | Würfle zwei Patronssegen aus und wähle einen davon, den du behältst. |
| 12 | Wähle unbeschränkt ein Talent aus. |


### Titel

| Level | Rechtschaffen | Chaotisch | Neutral |
| :--- | :--- | :--- | :--- |
| 1-2 | Begünstigter | Gezeichneter | Erwählter |
| 3-4 | Herold | Eiferer | Kanalisator |
| 5-6 | Eminent | Okkultist | Prophezeiter |
| 7-8 | Erhabener | Champion | Transzendenter |
| 9-10 | Inkarnation | Vorbote | Avatar |
